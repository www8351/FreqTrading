//+------------------------------------------------------------------+
//|                                                 SmcUs100_EA.mq5   |
//|      Multi-timeframe Smart Money Concepts A+ engine for US100     |
//|                                                                  |
//|  Self-contained port of the Python orb/smc/ package (magic       |
//|  20260622). Zero DLLs, zero custom includes — only the stock     |
//|  <Trade/Trade.mqh>. Copy into <terminal>/MQL5/Experts/, compile  |
//|  (F7), attach to a US100.ecn M15 chart on DEMO.                   |
//|                                                                  |
...
//|     2% risk sizing; layered partials 5R/7R + 10R runner; then a    |
//|     TWO-STAGE discrete SL (max 2 tighten-only modifies/position):  |
//|     stage1 = breakeven+costs at +1R, stage2 = lock candle-N low/   |
//|     high (floored to +1R) at +2R, then FROZEN. Both gated on the   |
//|     M1 N+1 rule (candidate = prior closed M1 bar, confirmed by the |
//|     just-closed one). Ports orb/smc/exits.py LadderExitManager.    |
...
//+------------------------------------------------------------------+
// NOTE: 1:1 clone of SmcXau_EA.mq5 -- engine body is identical. Make engine
//       edits in SmcXau_EA.mq5, then re-propagate. ONLY the input block differs.
#property copyright "FreqTrading — SMC US100"
#property version   "2.00"   // two-stage discrete SL (D-029/D-034 parity)
#property strict
...
input long    InpMagic            = 20260622;   // magic (US100 SMC EA)
...
input ENUM_TIMEFRAMES InpTriggerTf = PERIOD_M15; // SMC trigger TF (Python trigger_tf_min;
//                                                  US100 live keeper ran M15). HTF=H4/D1 fixed.
...
input double  InpPocTol           = 14.0;       // distance to POC = htf_poi (index pts)
input int     InpTicksPerRow      = 700;        // profile row = ticks*point
...
input double  InpStopBuffer       = 3.5;        // beyond invalidation (index pts)
input double  InpStopMaxDist      = 105.0;      // cap; wider structural stop = skip
...
input double  InpFinalTpR         = 10.0;       // close remainder here (0 = runner rides SL out)
input double  InpStage1AtR        = 1.0;        // stage1 (breakeven+costs) SL trigger, in R
input double  InpStage2AtR        = 2.0;        // stage2 (profit-lock) SL trigger, in R (> stage1)
input double  InpStage2MinLockR   = 1.0;        // stage2 SL floor, in R from entry (never looser)
input double  InpStage2Buffer     = 3.5;        // beyond candle-N low/high, stage2 lock (= InpStopBuffer)
input double  InpCommPerLot       = 0.0;        // $ round-trip comm/lot for stage1 cost buffer
//                                                 (live keeper uses 0; SmcConfig default is 7.0)
...
datetime g_last_m15_bar   = 0;    // new-trigger-bar gate (TF = g_trig_tf)
datetime g_last_m1_bar    = 0;    // new-M1-bar gate (exit management)
ENUM_TIMEFRAMES g_trig_tf = PERIOD_M15;  // SMC trigger TF, resolved from InpTriggerTf in OnInit
...
int OnInit()
{
   ...
   g_trig_tf = InpTriggerTf;                // SMC trigger timeframe (M15/M30/...)
...
void OnTick()
{
   ...
   datetime m15t = iTime(_Symbol, g_trig_tf, 0);
...
void TryEnter()
{
   ...
   StructResult m15 = ScanStructure(g_trig_tf, InpLookbackBars);
   ...
   double dO = iOpen (_Symbol, g_trig_tf, 1);
   double dH = iHigh (_Symbol, g_trig_tf, 1);
   double dL = iLow  (_Symbol, g_trig_tf, 1);
   double dC = iClose(_Symbol, g_trig_tf, 1);
   long   dV = iVolume(_Symbol, g_trig_tf, 1);

   double atr_m15 = AtrOnTf(g_trig_tf, InpAtrPeriod);
...
bool CheckDisplacement(double o, double h, double l, double c, long vol, double atr)
{
   ...
   double vsma = VolumeSmaOnTf(g_trig_tf, InpVolSmaPeriod);
   if(vol > 0 && vsma > 0.0)
...
bool CheckCisd(int bias, double dC)
{
   ...
   double pO = iOpen (_Symbol, g_trig_tf, 2);
   double pC = iClose(_Symbol, g_trig_tf, 2);
   double p2C= iClose(_Symbol, g_trig_tf, 3);
   if(bias == DIR_LONG)
...
{
   MqlRates rates[];
   int copied = CopyRates(_Symbol, g_trig_tf, from, to, rates);
...
//====================================================================
//  Exit management — layered partials + TWO-STAGE discrete SL.
//  Ports orb/smc/exits.py LadderExitManager. Stateless: partial state is
//  inferred from closed volume (deal history) and stage state from where the
//  live SL sits vs entry, so a terminal restart loses nothing. Runs once per
//  new M1 bar: the just-closed bar (index 1) is X (the N+1 confirmation), the
//  candidate N is the prior closed bar (index 2).
//====================================================================
void ManageOpenPositions()
{
   ...
   for(int idx = total-1; idx >= 0; idx--)
   {
      ...
      bool   is_long = (dir == DIR_LONG);
      double entry   = PositionGetDouble(POSITION_PRICE_OPEN);
      double sl      = PositionGetDouble(POSITION_SL);
      double vol_now = PositionGetDouble(POSITION_VOLUME);

      // Risk distance d from the ORIGINAL (opening-order) SL, never the live
      // SL -- the live SL moves once stage1/2 tighten it, which would shrink d
      // and corrupt every R. Mirrors the Python OriginalStopFromHistory fix
      // (D-029). No stop on record -> cannot size R -> skip.
      double orig_sl = OriginalStopFromHistory(ticket, sl);
      double d = (orig_sl > 0.0) ? MathAbs(entry - orig_sl) : 0.0;
      if(d <= 0.0) continue;

      // "current price" = the just-closed M1 bar X (Python candle.close).
      double curC = iClose(_Symbol, PERIOD_M1, 1);
      double r    = is_long ? (curC - entry)/d : (entry - curC)/d;

      double vol0 = OriginalVolumeFromHistory(ticket, vol_now);

      // ---- partials (real-time on X.close), state inferred from closed vol --
      ...
      if(!p1_done && r >= InpPartialR1)
      {
         ...
         if(v > 0.0 && (vol_now - v) >= MinVol() - 1e-9) trade.PositionClosePartial(ticket, v);
      }
      ...
      // ---- two-stage discrete SL, gated on the confirmed candle N ----------
      // Stage state inferred STATELESSLY from where the live SL sits:
      //   long : SL >= entry+minLock*d -> stage2 locked (frozen); SL >= entry -> stage1 done
      //   short: mirrors. stage2_done gates the block (SL frozen after the lock).
      double lock2 = is_long ? (entry + InpStage2MinLockR*d)
                             : (entry - InpStage2MinLockR*d);
      bool stage2_done = is_long ? (sl >= lock2 - _Point/2)
                                 : (sl > 0.0 && sl <= lock2 + _Point/2);
      bool stage1_done = is_long ? (sl >= entry - _Point/2)
                                 : (sl > 0.0 && sl <= entry + _Point/2);

      if(!stage2_done)
      {
         double nC = iClose(_Symbol, PERIOD_M1, 2);   // candidate N = prior closed M1 bar
         double nL = iLow  (_Symbol, PERIOD_M1, 2);
         double nH = iHigh (_Symbol, PERIOD_M1, 2);
         double r_n = is_long ? (nC - entry)/d : (entry - nC)/d;

         double new_sl = 0.0;
         if(r_n >= InpStage2AtR)
         {
            // stage2: lock candle-N extreme +/- buffer, floored to min-lock R
            new_sl = is_long ? MathMax(nL - InpStage2Buffer, lock2)
                             : MathMin(nH + InpStage2Buffer, lock2);
         }
         else if(!stage1_done && r_n >= InpStage1AtR)
         {
            // stage1: breakeven + costs (spread + commission/lot / value_per_move)
            double spread = SymbolInfoDouble(_Symbol, SYMBOL_ASK)
                          - SymbolInfoDouble(_Symbol, SYMBOL_BID);
            double vpp    = ValuePerPrice();
            double buf    = spread + (vpp > 0.0 ? InpCommPerLot/vpp : 0.0);
            new_sl = is_long ? (entry + buf) : (entry - buf);
         }

         if(new_sl > 0.0)
         {
            bool tighter = IsStrictlyTighter(dir, sl, new_sl);
            bool valid   = is_long ? (new_sl < curC) : (new_sl > curC);
            if(tighter && valid)
               trade.PositionModify(ticket, NormalizeDouble(new_sl, _Digits), 0.0);
            // else: skip silently; re-evaluated on a later M1 bar
         }
      }
   }
}

//  $ value of a 1.0 price move per 1.0 lot (Python SmcConfig value_per_move).
...
//  Original structural SL = the opening ORDER's SL (deals carry no SL, so
//  resolve the opening deal's order and read ORDER_SL). d = |entry - this| is
//  stable even after stage1/2 tighten the live position SL (D-029 fix).
double OriginalStopFromHistory(ulong position_id, double fallback)
{
   if(!HistorySelectByPosition(position_id)) return fallback;
   int deals = HistoryDealsTotal();
   for(int i = 0; i < deals; i++)
   {
      ...
      ulong ord = (ulong)HistoryDealGetInteger(dticket, DEAL_ORDER);
      if(HistoryOrderSelect(ord))
      {
         double s = HistoryOrderGetDouble(ord, ORDER_SL);
         if(s > 0.0) return s;
      }
      break;   // opening deal handled
   }
   return fallback;
}
