//+------------------------------------------------------------------+
//|                                                   SmcXag_EA.mq5   |
//|      Multi-timeframe Smart Money Concepts A+ engine for XAGUSD    |
//|                                                                  |
//|  Self-contained port of the Python orb/smc/ package (magic       |
//|  20260624). Zero DLLs, zero custom includes — only the stock     |
//|  <Trade/Trade.mqh>. Copy into <terminal>/MQL5/Experts/, compile  |
//|  (F7), attach to an XAGUSD.ecn M15 chart on DEMO.                 |
//|                                                                  |
...
#property copyright "FreqTrading — SMC XAG"
...
input long    InpMagic            = 20260624;   // magic (XAGUSD SMC EA)
...
//                                                  XAGUSD live keeper ran M15). HTF=H4/D1 fixed.
...
input double  InpPocTol           = 0.03;       // $ distance to POC = htf_poi
input int     InpTicksPerRow      = 3;          // profile row = ticks*point
...
input double  InpStopBuffer       = 0.02;       // $ beyond invalidation
input double  InpStopMaxDist      = 0.4;        // $ cap; wider structural stop = skip
...
input double  InpStage2Buffer     = 0.02;       // $ beyond candle-N low/high, stage2 lock (= InpStopBuffer)
