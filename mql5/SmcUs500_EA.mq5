//+------------------------------------------------------------------+
//|                                                 SmcUs500_EA.mq5   |
//|      Multi-timeframe Smart Money Concepts A+ engine for US500     |
//|                                                                  |
//|  Self-contained port of the Python orb/smc/ package (magic       |
//|  20260623). Zero DLLs, zero custom includes — only the stock     |
//|  <Trade/Trade.mqh>. Copy into <terminal>/MQL5/Experts/, compile  |
//|  (F7), attach to a US500.ecn M15 chart on DEMO.                   |
//|                                                                  |
...
#property copyright "FreqTrading — SMC US500"
...
input long    InpMagic            = 20260623;   // magic (US500 SMC EA)
...
//                                                  US500 live keeper ran M15). HTF=H4/D1 fixed.
...
input double  InpPocTol           = 3.6;        // distance to POC = htf_poi (index pts)
input int     InpTicksPerRow      = 180;        // profile row = ticks*point
...
input double  InpStopBuffer       = 0.9;        // beyond invalidation (index pts)
input double  InpStopMaxDist      = 27.0;       // cap; wider structural stop = skip
...
input double  InpStage2Buffer     = 0.9;        // beyond candle-N low/high, stage2 lock (= InpStopBuffer)
