//+------------------------------------------------------------------+
//|                                                   SmcBtc_EA.mq5   |
//|      Multi-timeframe Smart Money Concepts A+ engine for BTCUSD    |
//|                                                                  |
//|  Self-contained port of the Python orb/smc/ package (magic       |
//|  20260625). Zero DLLs, zero custom includes — only the stock     |
//|  <Trade/Trade.mqh>. Copy into <terminal>/MQL5/Experts/, compile  |
//|  (F7), attach to a BTCUSD.ecn M15 chart on DEMO.                  |
//|                                                                  |
...
#property copyright "FreqTrading — SMC BTC"
...
input long    InpMagic            = 20260625;   // magic (BTCUSD SMC EA)
...
//                                                  BTCUSD live keeper ran M15). HTF=H4/D1 fixed.
...
input double  InpPocTol           = 60.0;       // $ distance to POC = htf_poi
input int     InpTicksPerRow      = 3000;       // profile row = ticks*point
...
input double  InpStopBuffer       = 40.0;       // $ beyond invalidation
input double  InpStopMaxDist      = 1500.0;     // $ cap; wider structural stop = skip
...
input double  InpStage2Buffer     = 40.0;       // $ beyond candle-N low/high, stage2 lock (= InpStopBuffer)
