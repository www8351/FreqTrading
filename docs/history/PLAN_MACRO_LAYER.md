# PLAN — Macro / Fundamental Decision Layer
...
- **תהליך אחד לסימבול, ללא state משותף** (STRATEGY.md §1). מאקרו הוא גלובלי
...
- **כבר קיים PLANNED רלוונטי ב-STRATEGY.md:** `news_modifier` (`news_sl_multiplier`,
...
מ-STRATEGY.md: 30 דק' לפני/אחרי FOMC/NFP/CPI (מתיישר עם `pre_market_blackout`
...
| `custom_stake_amount` / sizing | `cfg.qty` מועבר ל-`broker.execute` | מכפיל דינמי: `news_lot_divider` (STRATEGY.md §2) — חצי lot ב-impact גבוה כדי לשמור 5% סיכון |
...
backtest מובהק. זה מתיישר עם הדיסציפלינה ב-STRATEGY.md (כל שינוי מגובה backtest).
...
`orb/cli.py`, `STRATEGY.md`, `README.md`.
